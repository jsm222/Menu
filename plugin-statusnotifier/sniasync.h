...
class SniAsync : public QObject
{
    Q_OBJECT
public:
    SniAsync(const QString &service, const QString &path, const QDBusConnection &connection,
                ...
                [this, finished, name](QDBusPendingCallWatcher *call) {
                    ...
                    if (reply.isError())
                        qDebug().noquote().nospace()
                                << "Error on DBus request(" << m_sni.service() << ','
                                << m_sni.path() << "): " << reply.error();
                    finished(qdbus_cast<typename std::function<
                                     typename call_signature<F>::type>::argument_type>(
                            reply.value()));
                    call->deleteLater();
                });
    }
...
public slots:
    ...
    {
        return m_sni.Scroll(delta, orientation);
    }
    inline QDBusPendingReply<> SecondaryActivate(int x, int y)
    {
        return m_sni.SecondaryActivate(x, y);
    }
